...
- (void)viewDidLoad
{
    ...
    MoreModel *numnum=[[MoreModel alloc] init];
    [numnum setName:[NSString stringWithFormat:@"V%@",versionString]];
    [numnum setType:MoreNum];
    [num addObject:numnum];
    [numnum release];

    if (IOS7) {
        self.sectionlist=[NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:loginlist,notiflist,applist,clearlist,num, nil] forKeys:[NSArray arrayWithObjects:@"用户信息管理",@"通知管理",@"App推荐",@"缓存管理",@"版本信息", nil]];
    }
    else
    {
